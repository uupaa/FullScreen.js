...
var FULLSCREEN_EVENTS = {
    "stable":   { "change": "fullscreenchange",       "error": "fullscreenerror"       },
    "webkit":   { "change": "webkitfullscreenchange", "error": "webkitfullscreenerror" },
    "moz":      { "change": "mozfullscreenchange",    "error": "mozfullscreenerror"    },
    "ms":       { "change": "MSFullscreenChange",     "error": "MSFullscreenError"     },
};
...
function _detectVendor() { // @ret String
    ...
    if (vendor) {
        document["addEventListener"](FULLSCREEN_EVENTS[vendor]["change"], _handleEvent); // fullscreenchange
        document["addEventListener"](FULLSCREEN_EVENTS[vendor]["error"],  _handleEvent); // fullscreenerror
    }
    return vendor;
}

function _handleEvent(event) { // @arg Event
    var type = event.type === FULLSCREEN_EVENTS[_vendor]["change"] ? "change"
             : event.type === FULLSCREEN_EVENTS[_vendor]["error"]  ? "error"
                                                                   : "";
    if (type) {
...
function FullScreen_release() { // @desc cleanup, release all events and event listeners.
    ...
    if (_vendor) {
        ...
        document["removeEventListener"](FULLSCREEN_EVENTS[_vendor]["change"], _handleEvent);
        document["removeEventListener"](FULLSCREEN_EVENTS[_vendor]["error"],  _handleEvent);
    }
}

function FullScreen_on(type,       // @arg String - "change" or "error"
...
//{@dev
    if (VERIFY) {
        ...
        $valid($some(type,     "change|error"), FullScreen_on, "type");
...
function FullScreen_off(type,       // @arg String - "change" or "error"
...
//{@dev
    if (VERIFY) {
        ...
        $valid($some(type,     "change|error"), FullScreen_off, "type");
